...
const Interface = () => {
  ...
  const selectionText = () => {
    if(purchaseStatus == status.NO_SELECTION){
      return <h3 className='Selection-Text-Moving'>Please make your selection</h3>
    }else if(purchaseStatus == status.ITEM_SELECTED){
      return <h3 className='Selection-Text-Moving'>You have selected Item {itemSelected}
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;The cost is ${cost}</h3>
    }else if(purchaseStatus == status.PAYMENT_IN_PROGRESS){
      return <h3 className='Selection-Text-Moving'>Please enter in ${cost} or more</h3>
    }
  }
  ...
  const inputPayment = (value : string) => {
  ...
  useEffect(()=> {
    const timer = setTimeout(() => setPurchaseStatus(status.PAYMENT_IN_PROGRESS), 8800)
